...
#Using fusions that have associated dependencies as a stratifying biomarker, 1) Determine which cell lines have COSMIC cancer census genes (not fusion partners or other TAD genes) with hotspot mutations and 2) Which of these hotspot mutations appear to lead to dependencies
```{r}

cosmic_census_gene_symbols <- cosmic_census_genes$`Gene Symbol`

canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies <- tibble(Fusion = NA, Cell_line = NA, Other_driver = NA, Dependency_probability_difference = NA, Negative_log10_BH_QValue = NA, Significant_dependency = NA)

counter = 0

for(fusion in unique(TAD_signficant_dependency_fusions_and_cell_lines_ALL_with_boundaries_duplicates_removed$Fusion)){
  
cell_lines_with_fusion_dependency <- ALL_fusions_protein_coding_filtered_list_final %>% dplyr::filter(Fusion == fusion) %>% distinct(DepMap_ID) %>% unlist() %>% unname() %>% intersect(colnames(ccle_dependency_probability_matrix))

cosmic_census_genes_excluding_fusion_associated <- setdiff(cosmic_census_gene_symbols,c(left_gene_TAD_neighbors[[fusion]], right_gene_TAD_neighbors[[fusion]], fusion_partners[[fusion]]))

for(cell_line in cell_lines_with_fusion_dependency){
  
cosmic_census_genes_with_hotspot_mutation <- CCLE_mutations_annotated %>% filter(DepMap_ID == cell_line & Hugo_Symbol %in% cosmic_census_genes_excluding_fusion_associated & isTCGAhotspot == TRUE) %>% distinct(Hugo_Symbol) %>% unlist() %>% unname()  
  
for(other_driver in cosmic_census_genes_with_hotspot_mutation){

if(other_driver %in% ccle_dependency_probability_matrix_df$gene){
  
if(!subset(dependency_probability_fusion_with_vs_without_stats[[fusion]], subset = gene == other_driver, select = dependency_probability_difference) %>% unlist() %>% unname() == "NaN"){
  
dependency_probability_difference <-  subset(dependency_probability_fusion_with_vs_without_stats[[fusion]], subset = gene == other_driver, select = dependency_probability_difference) %>% unlist() %>% unname()

negative_log10_BH_QValue <-  subset(dependency_probability_fusion_with_vs_without_stats[[fusion]], subset = gene == other_driver, select = negative_log10_BH_QValue) %>% unlist() %>% unname()

canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies <- add_row(canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies, Fusion = fusion, Cell_line = cell_line, Other_driver = other_driver, Dependency_probability_difference = dependency_probability_difference, Negative_log10_BH_QValue = negative_log10_BH_QValue, Significant_dependency = dependency_probability_difference > 0.5 & negative_log10_BH_QValue > -log10(.05))
...
canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies <- canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies %>% remove_missing()

cell_lines_with_fusion_associated_dependencies_with_at_least_one_hotspot_driver <- canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies %>% distinct(Cell_line)

cell_line_hotspot_mutation_combinations_for_cell_lines_with_fusion_associated_dependencies_with_at_least_one_hotspot_driver <- canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies %>% distinct(Cell_line,Other_driver)

other_driver_count_by_cell_line <- canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies %>% group_by(Cell_line) %>% summarise(other_driver_count = n_distinct(Other_driver))
...
cell_lines_with_fusion_associated_dependencies_with_at_least_one_hotspot_driver_that_translates_to_a_dependency <- canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies %>% filter(Significant_dependency == TRUE) %>% distinct(Cell_line)

write_tsv(canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies, here::here("results/datatables","canonical_driver_mutations_and_dependencies_in_fusion_contexts_with_associated_dependencies.tsv"))
